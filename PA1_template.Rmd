...
```
- The mean total daily steps is **`r format(mean_daily_steps)`**.
- The median total daily steps is **`r format(median_daily_steps)`**.
...
```
- Interval **#`r max_avg_interval`** has the maximum average number of steps (`r format(max_avg_steps, digits=5)` steps).
...
```
- The dataset contains **`r na_steps`** missing step values, **`r na_interval`** missing interval values, and **`r na_date`** missing date values.  
...
```
- The mean total daily steps is **`r format(mean_daily_steps_imp)`**.
- The median total daily steps is **`r format(median_daily_steps_imp)`**.
...
```
- The change in the mean value is **`r format(change_in_mean_val)`**.
- The change in the median value is **`r format(change_in_median_val)`**.
